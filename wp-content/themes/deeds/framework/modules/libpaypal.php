...
class Paypal {
	...
	public function handleNotification( $vars = null ) {
		...
		if ( is_object( $handled ) ) {
			if ( !$handled->ok || $this->settings->logNotifications ) {
				$this->error( ($verified ? '' : 'Unverified ') . ($handled->ok ? '' : 'Unhandled ') . 'paypal notification ' . $this->newurlPairs( $vars ) );
			}
		}
		return $handled;
	}
	...
	private function callPaypalNVP( $method, $params ) {
		...
		$data = "METHOD=" . urlencode( $method ) . '&' . $this->newurlPairs( $headerParams ) . '&' . $this->newurlPairs( $params );
	...
	private function verifyNotification( $vars ) {
		...
		$data = 'cmd=_notify-validate&' . $this->newurlPairs( $vars );
		$response = $this->makeRequest( $url, $data );
		if ( $response === false ) {
			return false;
		}
	 ...
	 * @return string
	 */
	private function newurlPairs( $array ) {
		if ( is_admin() ) {
			return;
		}
