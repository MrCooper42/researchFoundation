...
VP_AutoLoader::add_directories(get_template_directory().'/framework/vp_new', 'VP_');
// using your own namespace
VP_AutoLoader::add_namespaces('MY_');
VP_AutoLoader::add_directories(get_template_directory().'/framework/vp_new/classes', 'MY_');
...
$vpfs->add_directories('views'   , get_template_directory().'/framework/vp_new/views');
...
function sh_admin_custom_scripts()
{
	...
	if( ( $pagenow == 'themes.php' && sh_set( $_GET, 'page' ) == 'wp_deeds'.'_option') || sh_set( $_REQUEST, 'action' ) == 'vp_ajax_wp_genuine_theme_options_save' )			
	{
		wp_enqueue_script('vp-metabox', get_template_directory_uri() . '/framework/vafpress/public/js/metabox.min.js', '', '', true);
	}
}
...
{
	//if( ($pagenow == 'themes.php' && sh_set( $_GET, 'page' ) == 'wp_deeds'.'_option') || sh_set( $_REQUEST, 'action' ) == 'vp_ajax_wp_genuine_theme_options_save' )
	require_once get_template_directory().'/framework/vp_new/classes/alchemy.php';
}
