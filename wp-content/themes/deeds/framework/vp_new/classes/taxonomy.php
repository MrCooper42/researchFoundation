...
class WPAlchemy_Taxonomy {
	...
	function __construct( $arr ) {
		...
		$this->types = array( 'post', 'page', 'product', 'cs_team', 'cs_events', 'cs_church', 'cs_ministry' );
	...
	function _init() {
		...
		if ( true ) {//$this->can_output())
			...
			add_action( 'edited_team_category', array( $this, '_save' ), 10, 2 );
			add_action( 'create_team_category', array( $this, '_save' ), 10, 2 );

			add_action( 'edited_events_category', array( $this, '_save' ), 10, 2 );
			add_action( 'create_events_category', array( $this, '_save' ), 10, 2 );

			add_action( 'edited_church_category', array( $this, '_save' ), 10, 2 );
			add_action( 'create_church_category', array( $this, '_save' ), 10, 2 );

			add_action( 'edited_ministry_category', array( $this, '_save' ), 10, 2 );
			add_action( 'create_ministry_category', array( $this, '_save' ), 10, 2 );
	...
	function vp_enqueue() {
		...
		if ( isset( $uri ) ) {
			...
			if ( $file == 'edit-tags.php' ) {
				$loader = VP_WP_Loader::instance();
				wp_register_script( 'select2', get_template_directory_uri() . '/framework/vafpress/public/js/vendor/select2.min.js', array(), '4.0', true );
				wp_register_script( 'cate', get_template_directory_uri() . '/framework/vafpress/public/js/vendor/category.js', array(), '4.0', true );
				wp_enqueue_style( 'select2', get_template_directory_uri() . '/framework/vafpress/public/css/vendor/select2.css', array(), '4.0', 'all' );
