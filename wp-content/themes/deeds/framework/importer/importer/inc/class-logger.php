...
if ( !class_exists( 'WP_Importer_Logger' ) ) {
	require CPATH . 'importer/WordPress-Importer/class-logger.php';
}
if ( !class_exists( 'WP_Importer_Logger_CLI' ) ) {
	require CPATH . 'importer/WordPress-Importer/class-logger-cli.php';
}

class wp_wpstore_Logger extends WP_Importer_Logger_CLI {
	...
	public function error_output( $level, $message, array $context = array() ) {
		...
		$this->error_output .= sprintf(
				'[%s] %s<br>', strtoupper( $level ), $message
		);
	}
