...
class wp_wpstore_Helpers {
    ...
    public static function download_import_files( $import_file_info, $start_date = '' ) {
        ...
        $upload_path      = apply_filters( 'wp_wpstore/upload_file_path', trailingslashit( $upload_dir['path'] ) );
        ...
        if ( empty( $import_file_info['import_file_url'] ) ) {
            if ( file_exists( $import_file_info['local_import_file'] ) ) {
                ...
                return new WP_Error(
                    'url_or_local_file_not_defined', sprintf(
                        __( '"import_file_url" or "local_import_file" for %s%s%s are not defined!', 'wp_deeds' ), '<strong>', $$import_file_info['import_file_name'], '</strong>'
                    )
                );
            }
        } else {
            ...
            $demo_import_file_path = $upload_path . apply_filters( 'wp_wpstore/downloaded_content_file_prefix', 'demo-content-import-file_' ) . $start_date . apply_filters( 'wp_wpstore/downloaded_content_file_suffix_and_file_extension', '.xml' );
        ...
        if ( !empty( $import_file_info['import_widget_file_url'] ) ) {
            ...
            $import_widgets_file_path = $upload_path . apply_filters( 'wp_wpstore/downloaded_widgets_file_prefix', 'demo-widgets-import-file_' ) . $start_date . apply_filters( 'wp_wpstore/downloaded_widgets_file_suffix_and_file_extension', '.json' );
        ...
        if ( !empty( $import_file_info['import_customizer_file_url'] ) ) {
            ...
            $import_customizer_file_path = $upload_path . apply_filters( 'wp_wpstore/downloaded_customizer_file_prefix', 'demo-customizer-import-file_' ) . $start_date . apply_filters( 'wp_wpstore/downloaded_customizer_file_suffix_and_file_extension', '.dat' );
    ...
    private static function get_content_from_url( $url, $file_name = 'Import file' ) {
        ...
        if ( empty( $url ) ) {
            return new WP_Error(
                'url_not_defined', sprintf(
                    __( 'URL for %s%s%s file is not defined!', 'wp_deeds' ), '<strong>', $file_name, '</strong>'
                )
            );
        }
        ...
        $response = wp_remote_get(
            $url, array( 'timeout' => apply_filters( 'wp_wpstore/timeout_for_downloading_import_file', 20 ) )
        );
        ...
        if ( is_wp_error( $response ) || 200 !== $response['response']['code'] ) {
            ...
            return new WP_Error(
                'file_fetching_error', sprintf(
                    __( 'An error occurred while fetching %s%s%s file from the server!%sReason: %s - %s.', 'wp_deeds' ), '<strong>', $file_name, '</strong>', '<br>', $response_error['error_code'], $response_error['error_message']
                ) . '<br>' .
                apply_filters( 'wp_wpstore/message_after_file_fetching_error', '' )
            );
        }
        ...
        if ( !$wp_filesystem->put_contents( $file_path, $content ) ) {
            return new WP_Error(
                'failed_writing_file_to_server', sprintf(
                    __( 'An error occurred while writing file to your server! Tried to write a file to: %s%s.', 'wp_deeds' ), '<br>', $file_path
                )
            );
        }
    ...
    public static function data_from_file( $file_path ) {
        ...
        if ( !$data ) {
            return new WP_Error(
                'failed_reading_file_from_server', sprintf(
                    __( 'An error occurred while reading a file from your server! Tried reading file from path: %s%s.', 'wp_deeds' ), '<br>', $file_path
                )
            );
        }
    ...
    private static function check_wp_filesystem_credentials() {
        ...
        if ( !( 'direct' === get_filesystem_method() ) ) {
            return new WP_Error(
                'no_direct_file_access', sprintf(
                    __( 'This WordPress page does not have %sdirect%s write file access. This plugin needs it in order to save the demo import xml file to the upload directory of your site. You can change this setting with these instructions: %s.', 'wp_deeds' ), '<strong>', '</strong>', '<a href="http://gregorcapuder.com/wordpress-how-to-set-direct-filesystem-method/" target="_blank">How to set <strong>direct</strong> filesystem method</a>'
                )
            );
        }

        // Get plugin page settings.
        $plugin_page_setup = apply_filters( 'wp_wpstore/plugin_page_setup', array(
            'parent_slug' => 'themes.php',
            'page_title'  => esc_html__( 'One Click Demo Import', 'wp_deeds' ),
            'menu_title'  => esc_html__( 'Import Demo Data', 'wp_deeds' ),
            'capability'  => 'import',
        ...
        if ( false === ( $creds = request_filesystem_credentials( $demo_import_page_url, '', false, false, null ) ) ) {
            return new WP_error(
                'filesystem_credentials_could_not_be_retrieved', __( 'An error occurred while retrieving reading/writing permissions to your server (could not retrieve WP filesystem credentials)!', 'wp_deeds' )
            );
        }
        ...
        if ( !WP_Filesystem( $creds ) ) {
            return new WP_Error(
                'wrong_login_credentials', __( 'Your WordPress login credentials don\'t allow to use WP_Filesystem!', 'wp_deeds' )
            );
        }
    ...
    public static function get_log_path( $start_date = '' ) {
        ...
        $log_path = $upload_path . apply_filters( 'wp_wpstore/log_file_prefix', 'log_file_' ) . $start_date . apply_filters( 'wp_wpstore/log_file_suffix_and_file_extension', '.txt' );
    ...
    public static function register_file_as_media_attachment( $log_path ) {
        ...
        $filetype  = wp_check_filetype( basename( $log_path ), apply_filters( 'wp_wpstore/file_mimes', $log_mimes ) );

        // Prepare an array of post data for the attachment.
        $attachment = array(
            'guid'           => self::get_log_url( $log_path ),
            'post_mime_type' => $filetype['type'],
            'post_title'     => apply_filters( 'wp_wpstore/attachment_prefix', esc_html__( 'One Click Demo Import - ', 'wp_deeds' ) ) . preg_replace( '/\.[^.]+$/', '', basename( $log_path ) ),
            'post_content'   => '',
            'post_status'    => 'inherit',
        );
    ...
    public static function get_log_url( $log_path ) {
        ...
        $upload_dir = wp_upload_dir();
        $upload_url = apply_filters( 'wp_wpstore/upload_file_url', trailingslashit( $upload_dir['url'] ) );
    ...
    public static function verify_ajax_call() {
        ...
        if ( !current_user_can( 'import' ) ) {
            wp_die(
                sprintf(
                    __( '%sYour user role isn\'t high enough. You don\'t have permission to import demo data.%s', 'wp_deeds' ), '<div class="notice  notice-error"><p>', '</p></div>'
                )
            );
        }
    }
    ...
    public static function process_uploaded_files( $uploaded_files, $log_file_path ) {
        ...
        if ( empty( $content_file_info['file'] ) || isset( $content_file_info['error'] ) ) {
            ...
            self::log_error_and_send_ajax_response(
                __( 'Please upload XML file for content import. If you want to import widgets or customizer settings only, please use Widget Importer & Exporter or the Customizer Export/Import plugin.', 'wp_deeds' ), $log_file_path, esc_html__( 'Upload files', 'wp_deeds' )
            );
        }
        ...
        if ( $widget_file_info && !isset( $widget_file_info['error'] ) ) {
            ...
            $log_added = self::append_to_file(
                    sprintf(
                        __( 'Widget file was not uploaded. Error: %s', 'wp_deeds' ), $widget_file_info['error']
                    ), $log_file_path, esc_html__( 'Upload files', 'wp_deeds' )
            );
        }
        ...
        if ( $customizer_file_info && !isset( $customizer_file_info['error'] ) ) {
            ...
            $log_added = self::append_to_file(
                    sprintf(
                        __( 'Customizer file was not uploaded. Error: %s', 'wp_deeds' ), $customizer_file_info['error']
                    ), $log_file_path, esc_html__( 'Upload files', 'wp_deeds' )
            );
        }
        ...
        $log_added = self::append_to_file(
                __( 'The import files were successfully uploaded!', 'wp_deeds' ) . self::import_file_info( $selected_import_files ), $log_file_path, esc_html__( 'Upload files', 'wp_deeds' )
        );
    ...
    public static function import_file_info( $selected_import_files ) {
        return PHP_EOL .
            sprintf(
                __( 'Initial max execution time = %s', 'wp_deeds' ), ini_get( 'max_execution_time' )
            ) . PHP_EOL .
            sprintf(
                __( 'Files info:%1$sSite URL = %2$s%1$sData file = %3$s%1$sWidget file = %4$s%1$sCustomizer file = %5$s', 'wp_deeds' ), PHP_EOL, get_site_url(), $selected_import_files['content'], empty( $selected_import_files['widgets'] ) ? esc_html__( 'not defined!', 'wp_deeds' ) : $selected_import_files['widgets'], empty( $selected_import_files['customizer'] ) ? esc_html__( 'not defined!', 'wp_deeds' ) : $selected_import_files['customizer']
        );
    }
    ...
    public static function log_error_and_send_ajax_response( $error_text, $log_file_path, $separator = '' ) {
        ...
        // Add this error to log file.
        $log_added = self::append_to_file(
                $error_text, $log_file_path, $separator
        );
