...
function sh_xml_importer() {
    if (isset($_POST['action']) && $_POST['action'] == 'theme-install-demo-data') {
        ...
        $fileUrl = 'https://s3.amazonaws.com/webinane/dummy/deeds/deeds.zip';
        ...
        if (file_exists(SH_ROOT . "framework/backup/".$demo."/data.xml")) {
            define('WP_LOAD_IMPORTERS', true);
            if (!class_exists('WP_Import')) {
                include ( SH_ROOT . 'framework/wordpress-importer/wordpress-importer.php');
            }
            $content_xml = SH_ROOT . "framework/backup/".$demo."/data.xml";
