...
class VP_Option extends WPAlchemy_MetaBox {
	...
	public function __construct( array $configs ) {
		...
		extract( $test = array_merge( array(
			...
			'layout' => 'fixed',
			'page_title' => __( 'Vafpress Options', 'wp_deeds' ),
			'menu_label' => __( 'Vafpress Options', 'wp_deeds' ),
			'priority' => 10,
				), $configs ) );
		...
		if ( isset( $option_key ) )
			...
			throw new Exception( __( 'Option Key is required', 'wp_deeds' ), 1 );
		if ( isset( $template ) )
			$this->set_template( $template );
		else
			throw new Exception( __( 'Template Array/File is required', 'wp_deeds' ), 1 );
		if ( isset( $page_slug ) )
			$this->set_page_slug( $page_slug );
		else
			throw new Exception( __( 'Page Slug is required', 'wp_deeds' ), 1 );
	...
	public function dev_mode_notice() {
		VP_WP_Util::admin_notice( __( "Development Mode is Active, options' values won't be saved into database.", 'wp_deeds' ), false );
	}
	...
	function ajax_import_option() {
		...
		if ( $result['status'] ) {
			...
			if ( empty( $option ) ) {
				$result['status'] = false;
				$result['message'] = __( "Can not be empty", 'wp_deeds' );
			} else {
				...
				if ( is_array( $option ) ) {
					...
					$result['status'] = false;
					$result['message'] = __( "Invalid data", 'wp_deeds' );
				}
			}
		}
	...
	function ajax_export_option() {
		...
		if ( $result['status'] ) {
			...
			$result = array(
				'status' => true,
				'message' => __( "Successful", 'wp_deeds' ),
				'option' => $sr_options,
			);
		}
	...
	function vp_verify_nonce() {
		...
		if ( $verify ) {
			$result['status'] = true;
			$result['message'] = __( "Successful", 'wp_deeds' );
		} else {
			$result['status'] = false;
			$result['message'] = __( "Unverified Access.", 'wp_deeds' );
		}
		return $result;
	}
	...
	public function init_options_set() {
		...
		if ( is_string( $this->get_template() ) and is_file( $this->get_template() ) )
			...
			throw new Exception( __( 'Invalid template supplied', 'wp_deeds' ), 1 );
		...
		if ( $this->use_util_menu() ) {
			...
			$util_menu->set_title( __( 'Utility', 'wp_deeds' ) );
			$util_menu->set_name( 'menu_util' );
			$util_menu->set_icon( 'font-awesome:fa fa-ambulance' );

			// setup restore default section
			$restore_section = new VP_Option_Control_Group_Section();
			$restore_section->set_title( __( 'Restore Default', 'wp_deeds' ) );
			...
			//$exim_section->set_title(__('Export/Import', 'wp_deeds'));
