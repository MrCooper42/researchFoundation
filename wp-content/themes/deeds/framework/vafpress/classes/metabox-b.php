...
class VP_Metabox extends WPAlchemy_MetaBox
{
	...
	function __construct($arr)
	{
		...
		if( $this->is_dev_mode )
		{
			$this->title = __('[Development Mode] ', 'wp_deeds') . $this->title;
		}
	...
	function _render_repeating_group($group)
	{
		...
		foreach ($group['groups'] as $g)
		{
			...
			$html .= '<div class="vp-wpa-group-heading"><a href="#" class="vp-wpa-group-title">' . $icon . $group['title'] . '</a><a href="#" class="dodelete vp-wpa-group-remove" title="'. __('Remove', 'wp_deeds') .'"><i class="icon-remove"></i> '. __('Remove', 'wp_deeds') .'</a></div>';
			$html .= '<div class="vp-controls' . ((!$is_first) ? ' vp-hide' : '') . '">';
		...
		$html .= '<a href="#" class="button button-large docopy-' . $name . '">'. __('Add More', 'wp_deeds') . ' ' . $group['title'] . '</a>';
		$html .= '</div>';
	...
	function _save($post_id) 
	{
		...
		else
		{
			...
			else
			{
				...
				/*$custom_date_format = new DateTime(sh_set(sh_set(sh_set($new_data, 'sh_event_options'), 0), 'start_date'));
				if ($post_type == 'cs_events' && sh_set(sh_set(sh_set($new_data, 'sh_event_options'), 0), 'start_date') != '') {
					update_post_meta($post_id, 'sh_event_start_date', $custom_date_format->format('d/m/Y'));
				}*/
