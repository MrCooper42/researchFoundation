...
VP_Security::instance()->whitelist_function('vp_dep_is_soundcloud');

function vp_dep_is_soundcloud($value)
{
	if($value === 'soundcloud')
		return true;
	return false;
...
VP_Security::instance()->whitelist_function('vp_dep_is_own');

function vp_dep_is_own($value)
{
	if($value === 'own')
		return true;
	return false;
...
VP_Security::instance()->whitelist_function('vp_dep_is_direct');

function vp_dep_is_direct($value)
{
	if($value === 'direct')
		return true;
	return false;
...
function vp_font_preview($face, $style, $weight, $size, $line_height)
{
	...
	$dom .= __( 'Grumpy wizards make toxic brew for the evil Queen and Jack', 'wp_deeds') . '</p>';
