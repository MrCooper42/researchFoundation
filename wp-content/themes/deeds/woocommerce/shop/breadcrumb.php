...
<span class="category">
	...
	if ( (!is_home() && !is_front_page() && !( is_post_type_archive() && get_option( 'page_on_front' ) == woocommerce_get_page_id( 'shop' ) ) ) || is_paged() ) {
		...
		if ( is_category() ) {
			...
			echo $prepend . $before . __( 'Products tagged &ldquo;', 'wp_deeds' ) . $queried_object->name . '&rdquo;' . $after;
		} elseif ( is_day() ) {
			...
			if ( is_search() ) {
				...
				echo $before . '<a href="' . get_post_type_archive_link( 'product' ) . '">' . $_name . '</a>' . $delimiter . __( 'Search results for &ldquo;', 'wp_deeds' ) . get_search_query() . '&rdquo;' . $after;
			} elseif ( is_paged() ) {
			...
			echo $before . __( 'Error 404', 'wp_deeds' ) . $after;
			...
			echo $before . __( 'Search results for &ldquo;', 'wp_deeds' ) . get_search_query() . '&rdquo;' . $after;
		} elseif ( is_tag() ) {

			echo $before . __( 'Posts tagged &ldquo;', 'wp_deeds' ) . single_tag_title( '', false ) . '&rdquo;' . $after;
		} elseif ( is_author() ) {

			$userdata = get_userdata( $author );
			echo $before . __( 'Author:', 'wp_deeds' ) . ' ' . $userdata->display_name . $after;
		}

		if ( get_query_var( 'paged' ) )
			echo ' (' . __( 'Page', 'wp_deeds' ) . ' ' . get_query_var( 'paged' ) . ')';
